			...
			success: function(response) {
				...
				$.each(response, function(key, value) {
					value.forEach(flight => {
						...
						table.append('<tr><td>'+ flight.airline +'</td><td>' + flight.flightnumber + '</td><td>' + flight.origin.city +'</td><td></td><td>'+departureTime+'</td><td>' + arrivalTime + '</td><td>' + flight.status + '</td></tr>')
						console.log(flight);
					});
				});
			}
